...
def main():
    ...
    for lang in ['de', 'en', 'es', 'fr']:
        lang_amazon = load_dataset("amazon_reviews_multi", lang)
        dataset = lang_amazon.map(compute_bin_label)
        dataset = dataset.filter(lambda example: example['bin_label'] != -1)

        if lang != 'en':
            translator_map = partial(compute_translation, lang=lang)
            for subset in ['validation', 'test']:
                print(f"WORK WITH {lang} {subset}")
                dataset[subset] = dataset[subset].map(translator_map, batched=True)
            dataset['train'] = dataset['train'].map(id_translation, batched=True)
        else:
            dataset = dataset.map(id_translation, batched=True)

        dataset = dataset.map(tokenization_en, batched=True)
        ...
        for subset in subsets:
            normalize_dataset(dataset[subset])
        dataset.save_to_disk(f'amazon_tr_{lang}')
        print(dataset)
